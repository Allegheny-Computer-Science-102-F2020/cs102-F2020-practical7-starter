# cs102-F2020-practical7-starter
...
* [Practical Assignment Tasks](#practical-assignment-tasks)
  ...
  + [Using Nested Iteration to Compute the Cartesian Product of Two Sets](#using-nested-iteration-to-compute-the-cartesian-product-of-two-sets)
  + [Running a Test Suite for a Program that Computes the Cartesian Product of Two Sets](#running-a-test-suite-for-a-program-that-computes-the-cartesian-product-of-two-sets)
...
- To use a Docker container to run a Python program and observe its output
...
- To implement a function that calculates the Cartesian product of two sets
- To implement a Pytest test cases that test a set theoretic function
...
`Allegheny-Computer-Science-102-Fall-2020/computer-science-102-fall-2020-practical-7-gkapfham`.
...
```
git clone git@github.com:Allegheny-Computer-Science-102-F2020/computer-science-102-fall-2020-practical-7-gkapfham.git
```
...
assignment, you need to install the dependencies for the `cartesiancreator`
program that you will implement, debug, and observe. To complete this step you
need to type `cd cartesiancreator` and then `poetry install`. What output did
...
Python program you need to be in the directory called `cartesiancreator`.

### Using Nested Iteration to Compute the Cartesian Product of Two Sets

For this practical assignment you are going to design, implement, and test a
program that can create the Cartesian Product of two input sets. To learn more
about the Cartesian produce, please make sure that you read page 127 in the book
called "Doing Math with Python". This assignment will required you to combine
your knowledge of functions, iteration constructs, the list discrete structure,
the set discrete structure, and mathematics to create a Python program that you
implement in an industry-standard fashion. For instance, when you run the
completed version of the Python program with the command `poetry run python
cartesiancreator --first 1 --first 2 --second 3 --second 4` it will produce the
following output:
...
Address of generator for Cartesian produce of sets: <generator object cartesian_product at 0x7f497e9409e0>

Cartesian product of sets: [{1, 3}, {1, 4}, {2, 3}, {2, 4}]
```

After reading page 127 in the aforementioned book, you will see that the
Cartesian product operator involves the combination of every element in the
first set with every element in the second set. As part of this assignment you
must implement a doubly nested `for` loop structure in the `cartesian_product`
function inside of the `cartesian` module. This `cartesian_product` function
should produce one new member of the Cartesian product and then `yield` it to
the calling function. Before you go on to test and use this function, make sure
that you understand how this generating function uses the `yield` keyword and
why there are benefits to adopting this approach. Finally, students who are
interested in doing so should consider exploring other ways to implement this
function that do not rely on a generator-based approach. If you would like to
extend your solution further you could enhance the `__main__` file so that it
provides a command-line interface that would allow a person using the program to
pick which implementation of the Cartesian product computation that they want to
use. Finally, it is worth thinking about what happens when you run the program
with an empty set as one of the inputs to the Cartesian product. Can you explain
why the following output is correct?
...
Address of generator for Cartesian produce of sets: <generator object cartesian_product at 0x7fab23a829e0>

Cartesian product of sets: []
```

### Running a Test Suite for a Program that Computes the Cartesian Product of Two Sets

If you look in the `test_cartesian.py` file you will see a `TODO` marker asking
you to write a test case for the example given on page 127 in the book called
"Doing Math with Python" by Amit Saha. This test case should confirm that, if
the `cartesian_product` function is given as input the sets `first_set = set([1,
2])` and `second_set = set([3, 4])` then it should produce an output that
contains the sets `{1, 3}`, `{1, 4}`, `{2, 3}`, and `{2, 4}`. If both the test
suite and the `cartesian_product` function are correct, then you should be
able to run the test suite by typing `poetry run pytest` and see that it
produces the following output:

```
=========================== test session starts ============================
platform linux -- Python 3.8.5, pytest-5.4.3, py-1.9.0, pluggy-0.13.1
rootdir: /home/gkapfham/working/teaching/github-classroom/Allegheny-Computer-Science-102-F2020/solutions/cs102-F2020-practical7-solution/cartesiancreator
collected 4 items

tests/test_cartesian.py ....                                         [100%]
...
- The cartesian.py in cartesiancreator/cartesiancreator has at least 1 of the `first_element in first` fragment
- The cartesian.py in cartesiancreator/cartesiancreator has at least 1 of the `from typing import Iterator` fragment
- The cartesian.py in cartesiancreator/cartesiancreator has at least 1 of the `from typing import Set` fragment
- The cartesian.py in cartesiancreator/cartesiancreator has at least 1 of the `second_element in second` fragment
- The cartesian.py in cartesiancreator/cartesiancreator has at least 2 multiple-line Python comment(s)
- The cartesian.py in cartesiancreator/cartesiancreator has at least 3 single-line Python comment(s)
- The cartesian.py in cartesiancreator/cartesiancreator has exactly 0 of the `TODO` fragment
- The command `cd cartesiancreator; poetry install; poetry run python cartesiancreator --first 1 --first 2 --second 3 --second 4; cd ..` executes correctly
- The file cartesian.py exists in the cartesiancreator/cartesiancreator directory
- The file __main__.py exists in the cartesiancreator/cartesiancreator directory
- The file reflection.md exists in the writing directory
- The file test_cartesian.py exists in the cartesiancreator/tests directory
- The __main__.py in cartesiancreator/cartesiancreator has at least 2 multiple-line Python comment(s)
- The __main__.py in cartesiancreator/cartesiancreator has at least 4 single-line Python comment(s)
- The __main__.py in cartesiancreator/cartesiancreator has exactly 0 of the `TODO` fragment
- The __main__.py in cartesiancreator/cartesiancreator has exactly 1 of the `cartesian.cartesian_product` fragment
- The __main__.py in cartesiancreator/cartesiancreator has exactly 1 of the `import List` fragment
- The __main__.py in cartesiancreator/cartesiancreator has exactly 1 of the `run(main)` fragment
- The __main__.py in cartesiancreator/cartesiancreator has exactly 1 of the `set(first)` fragment
- The __main__.py in cartesiancreator/cartesiancreator has exactly 1 of the `set(second)` fragment
- The __main__.py in cartesiancreator/cartesiancreator has exactly 2 of the `Option([])` fragment
...
- The test_cartesian.py in cartesiancreator/tests has at least 1 multiple-line Python comment(s)
- The test_cartesian.py in cartesiancreator/tests has exactly 0 of the `TODO` fragment

```
        ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
        ┃ Passed 30/30 (100%) of checks for cs102-F2020-practical7! ┃
        ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
```
...
```
git remote add download git@github.com:Allegheny-Computer-Science-102-F2020/cs102-F2020-practical7-starter.git
```
